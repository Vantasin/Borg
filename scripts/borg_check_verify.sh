...
LOG_FILE="${LOG_FILE:-${LOG_DIR}/check_verify_$(date +%F).log}"
...
require_mounted() {
  ...
  if command -v findmnt >/dev/null 2>&1; then
    if ! findmnt -T "${path}" >/dev/null 2>&1; then
      echo "${label} (${path}) not mounted; skipping verify-data run."
      exit 0
    fi
  fi
}
...
if [ -n "${REPO_DATASET}" ]; then
  if ! command -v zfs >/dev/null 2>&1; then
    echo "zfs command not found; skipping verify-data run."
    exit 0
  fi

  if ! zfs list -H -o name "${REPO_DATASET}" >/dev/null 2>&1; then
    echo "ZFS dataset ${REPO_DATASET} unavailable; skipping verify-data run."
    exit 0
  fi
fi
...
if [ ! -d "${BORG_REPO}" ]; then
  echo "Borg repo ${BORG_REPO} unavailable; skipping verify-data run."
  exit 0
fi
...
############################################################
echo "===== Borg Check --verify-data Started: $(date) ====="
...
if [ ${CHECK_EXIT} -eq 0 ]; then
  send_mail \
    "[OK] Borg verify-data check on $(hostname)" \
    "Borg repository data verification completed successfully.
...
${LOG_SNIPPET}"
  ...
  echo "===== Borg Check --verify-data Finished Successfully: $(date) ====="
  exit 0
else
  send_mail \
    "[FAIL] Borg verify-data check on $(hostname)" \
    "Borg repository data verification FAILED.

Host: $(hostname)
Time: $(date)

borg check --verify-data exit code: ${CHECK_EXIT}

Log tail:
${LOG_SNIPPET}"

  echo "Email notification sent: FAILURE"
  echo "===== Borg Check --verify-data Finished With Errors: $(date) ====="
  exit 1
fi
